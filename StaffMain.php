...
if (!isset($_SESSION['lect_id'])) 
{
    ...
    echo '<script>window.location.href = "LoginForStaff.php";</script>';
	session_destroy();
    exit();
}

$staff_id = $_SESSION['lect_id'];
$query = "SELECT Lect_name FROM Lecturer WHERE Lect_id = '$staff_id'";
$name = mysqli_query($conn, $query);


$sql = "SELECT * FROM Lecturer L INNER JOIN Roles R ON L.Role_id = R.Role_id WHERE L.Lect_id = '$staff_id'";
